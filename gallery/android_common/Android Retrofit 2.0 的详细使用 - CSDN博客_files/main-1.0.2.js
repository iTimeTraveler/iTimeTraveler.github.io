...
feedlistScroll= function(){
    ...
    for(i = 0; i<feedlist.length; i++){="" feedchild="feedlist[i];" var="" childlisttop="$(feedlist[i]).offset().top;" feedlistget();="" if="" (($(feedlist[i]).attr('data-feed-show')="=="false")&&(currentBottom">=childListTop)&&!$(feedlist[i]).is(".tracking-click")) {
        ...
        if ($ === undef) {
            ...
            s.onload = s.onreadystatechange = function() {
                if (!done && (!this.readyState || this.readyState === 'loaded' || this.readyState === 'complete')) {
                    ...
                    $ = global.jQuery;
                    //设置IE跨域访问 兼容到包含9以下
                    if(window.navigator.appVersion.match(/MSIE (\d)*/i)&&Number(window.navigator.appVersion.match(/MSIE (\d)*/i)[1],10)</[^></feedlist.length;></caoyu#at#csdn.net>